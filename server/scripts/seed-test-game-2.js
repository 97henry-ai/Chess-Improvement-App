...
await db.query(
  `INSERT INTO games (username, chesscom_url, pgn, time_class, end_time, white, black,
   ...
   VALUES ($1, $2, $3, 'rapid', extract(epoch from now()), 'testuser', 'opponent', 1200, 1200, '0-1', 'white', 'loss', 'B01', 'Scandinavian Defense')
   ON CONFLICT (chesscom_url) DO NOTHING`,
  ['testuser', 'local-test-game-2', pgn]
);

const { rows } = await db.query('SELECT id FROM games WHERE chesscom_url = $1', ['local-test-game-2']);
